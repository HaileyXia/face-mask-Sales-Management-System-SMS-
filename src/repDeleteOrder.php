<?php
    ...
    //check if the order exceeds quota
    ...
    $leftn95 = $selectn95 + $quantityN95; //add quota back to orginal
    $leftsmask = $selectsmask + $quantityS;
    $leftsn95 = $selectsn95 + $quantitySN95;

    if($selectid != $employeeID) //check if the id enter match the employeeID
    {
    	$result = "invalid";
    }
    elseif($hour>24){ //check if the order over 24 hours
    	$result = "overtime";
    }
    elseif($excessnum == 0){ //check if the order exceeds quota
        $result = "excess";
    }
    else{ //delete the order and update the quota
    	$orderupdate = "DELETE FROM ordering WHERE orderingID ='{$deleteorder}' and employeeID = '{$employeeID}' ";
    	$sql1 = "UPDATE rep SET quotaN95='{$leftn95}', quotaSN95='{$leftsn95}', quotaS='{$leftsmask}' where employeeID ='{$employeeID}' " ;
